...
@Tag(name = "User", description = "API de gestión de usuarios y autenticación con rotación de tokens")
public class UserController {
    ...
    public ResponseEntity<ResponseUserRegister> register(@ModelAttribute RequestUserRegister request) {
       ...
       String result = this.userBusiness.register(request);
    ...
    public ResponseEntity<ResponseUserLogin> login(@RequestBody RequestUserLogin request) {
        ...
        if (user != null) {
            ...
            response.setUser(user); // Este DTO ya contiene los tokens generados en Business
    ...
    @Operation(summary = "Refrescar Tokens", description = "Recibe el refresh token actual y devuelve un nuevo par (Rotación)")
    public ResponseEntity<ResponseUserLogin> refresh(@RequestBody Map<String, String> request) {
        ResponseUserLogin response = new ResponseUserLogin();
        String oldRefreshToken = request.get("refreshToken");

        if (oldRefreshToken == null || oldRefreshToken.isEmpty()) {
            response.error();
            response.listMessage.add("El campo refreshToken es obligatorio");
            return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
        }

        DtoUser userWithNewTokens = this.userBusiness.refreshToken(oldRefreshToken);

        if (userWithNewTokens != null) {
            response.success();
            response.listMessage.add("Tokens actualizados correctamente");
            response.setUser(userWithNewTokens);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } else {
            response.error();
            response.listMessage.add("Refresh Token inválido, expirado o ya utilizado");
